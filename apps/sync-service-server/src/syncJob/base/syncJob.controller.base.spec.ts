...
const service = {
  ...
  syncJobs: () => FIND_MANY_RESULT,
  syncJob: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("SyncJob", () => {
  ...
  test("GET /syncJobs/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/syncJobs"}/${nonExistingId}`)
  ...
  test("GET /syncJobs/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/syncJobs"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /syncJobs existing resource", async () => {
